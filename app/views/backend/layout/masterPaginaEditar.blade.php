...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Constructos de páginas HTML</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!-- Loading Bootstrap -->
    {{HTML::style('backend/builder/bootstrap/css/bootstrap.css')}}

    <!-- Loading Flat UI -->
    {{HTML::style('backend/builder/css/flat-ui.css')}}
    
    {{HTML::style('backend/builder/css/style.css')}}
    
    {{HTML::style('backend/builder/css/spectrum.css')}}
    {{HTML::style('backend/builder/css/chosen.css')}}
    
    <link rel="shortcut icon" href="{{asset('backend/builder/images/favicon.ico')}}"> 
    
    <!-- Font Awesome -->
    {{HTML::style('backend/builder/css/font-awesome.css')}}
    
    {{HTML::style('backend/builder/js/redactor/redactor.css')}}

    <!-- HTML5 shim, for IE6-8 support of HTML5 elements. All other JS at the end of file. -->
    <!--[if lt IE 9]>
    	{{HTML::script('backend/builder/js/html5shiv.js')}}
		{{HTML::script('backend/builder/js/respond.min.js')}}
    <![endif]-->
  </head>
  <body>
  	<div class="menu" id="menu">
  		<div class="main" id="main">
  			<h3><span class="fui-list"></span> Secciones</h3>
  			<ul id="elements">
  				<li><a href="#" id="all">Todas las secciones</a></li>
  			</ul>
			<a href="#" class="toggle"><span class="fui-gear"></span></a>
  			<!--
  			<hr>
  			<h3><span class="fui-windows"></span> Páginas</h3>
  			<ul id="pages">
    ...
    <div class="container">
    	<header class="clearfix" data-spy="affix" data-offset-top="60" data-offset-bottom="200">
    		<a href="#" id="clearScreen" class="btn btn-danger btn-embossed pull-right disabled actionButtons"><span class="fui-trash"></span> Vaciar página</a>
			 <a href="{{\URL::to('/'.$paginaDatos->alias)}}" id="preview" target="_blank" class="btn btn-inverse btn-embossed pull-right disabled actionButtons" style="margin-right: 10px; display: none"><span class="fui-window"></span> Vista previa</a>
    		<!--<a href="#exportModal" id="exportPage" data-toggle="modal" class="btn btn-info btn-embossed pull-right disabled actionButtons"><span class="fui-export"></span> Exportar</a>-->
			<a href="#" id="savePage" class="btn btn-primary btn-embossed pull-right disabled actionButtons"><span class="fui-check"></span> <span class="bLabel">Sin cambios</span></a>
    		<div class="modes">
    			<b>Tipo de edición:</b>
    	...
    	<div class="screen" id="screen">
    		<div class="toolbar">
    			...
    			<div class="title" id="pageTitle">
    				<span><span>{{$paginaDatos->alias}}.html</span></span>
    			</div>
    		</div>
    		...
    		<div id="frameWrapper" class="frameWrapper empty">
    			...
    			<div class="start" id="start">
    				<span>Contruya su página arrastrando los elementos de la barra lateral hacia este espeacio</span>
    			</div>
    		</div>
    	</div><!-- /.screen -->
    </div><!-- /.container -->
    <div id="styleEditor" class="styleEditor">
    	...
    	<h3><span class="fui-new"></span> Editor detallado</h3>
    	<ul class="breadcrumb">
    		<li>Editar:</li>
    	...
    	<div class="tab-content">
          ...
          @if(isset($publicacionesLista) && count($publicacionesLista) > 0)
            <select class="select2">
              <option value="#">Seleccione una publicación</option>
              @foreach($publicacionesLista AS $publicacion)
                <option value="{{$publicacion['url']}}">{{$publicacion['titulo']}}</option>
              @endforeach
              </select>
              <br><br>
          ...
          @endif
        <!--
   				<select id="pageLinksDropdown"> <option value="#">Choose a block (one page sites)</option> </select>
   				<p class="text-center or"> <span>OR</span> </p>
          -->
 			...
 			<div class="tab-pane imageFileTab" id="image_Tab">
   				<label>Ruta completa de la imagen:</label>
   				<input type="text" class="form-control" id="imageURL" placeholder="Enter an image URL" value="">
   				<p class="text-center or"> <span>OR</span> </p>
   				<form id="imageUploadForm" action="../cargarPaginaImagen">
   				    	...
   				    	<div class="fileinput fileinput-new" data-provides="fileinput">
   							...
   							<div class="buttons">
   								<span class="btn btn-primary btn-sm btn-embossed btn-file">
   									<span class="fileinput-new" data-trigger="fileinput" ><span class="fui-image"></span>&nbsp;&nbsp;Seleccionar imagen</span>
    	...
    	<div class="sideButtons clearfix">
    		<!--<button type="button" class="btn btn-inverse btn-embossed btn-xs" id="cloneElementButton"><span class="fui-windows"></span> Clonar</button>-->
    		<button type="button" class="btn btn-warning btn-embossed btn-xs" id="resetStyleButton"><i class="fa fa-refresh"></i> Resetear</button>
    		<button type="button" class="btn btn-danger btn-embossed btn-xs" id="removeElementButton"><span class="fui-cross-inverted"></span>  Eliminar</button>
    	</div>
    ...
    <div id="hidden">
    	<iframe src="../../backend/builder/elements/skeleton.html" id="skeleton"></iframe>
    </div>
	<!-- modals -->
	<!-- export HTML popup -->
			...
			<div class="modal-dialog">
		    	<div class="modal-content">
		      		...
		      		<div class="modal-footer">
		        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal" id="exportCancel">Cancelar</button>
		        		<button type="submit" type="button" class="btn btn-primary btn-embossed" id="exportSubmit">Exportar</button>
		      		</div>
		    	</div><!-- /.modal-content -->
		  	</div><!-- /.modal-dialog -->
	  	</form>
		...
		<form action="preview.php" target="_blank" id="markupPreviewForm" method="post" class="form-horizontal">
			...
			<div class="modal-dialog">
		    	<div class="modal-content">
		      		...
		      		<div class="modal-footer">
		        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal" id="previewCancel">Cancelar</button>
		        		<button type="submit" type="button" class="btn btn-primary btn-embossed" id="showPreview">Mostrar vista previa</button>
		      		</div>
		    	</div><!-- /.modal-content -->
		  	</div><!-- /.modal-dialog -->
	  	</form>
	...
	<div class="modal fade small-modal" id="deleteBlock" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			¿Seguro que desea eliminar este bloque?
	      		</div><!-- /.modal-body -->
	      		<div class="modal-footer">
	...
	<div class="modal fade small-modal" id="resetBlock" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			¿Seguro que desea cargar los valores por default del bloque?
	      		</div><!-- /.modal-body -->
	      		<div class="modal-footer">
	        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal">Cancelar</button>
	        		<button type="button" type="button" class="btn btn-primary btn-embossed" id="resetBlockConfirm">Eliminar</button>
	      		</div>
	    	</div><!-- /.modal-content -->
	  	</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
	...
	<div class="modal fade small-modal" id="deleteAll" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			<p>¿Seguro que desea limpiar la página?</p>
              <p>Todos los bloques serán destruidos.</p>
	      		</div><!-- /.modal-body -->
	      		<div class="modal-footer">
	        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal">Cancelar</button>
	        		<button type="button" type="button" class="btn btn-primary btn-embossed" id="deleteAllConfirm">Limpiar</button>
	      		</div>
	    	</div><!-- /.modal-content -->
	  	</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
	...
	<div class="modal fade small-modal" id="deletePage" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			¿Seguro que desea eliminar esta página por completo?
	      		</div><!-- /.modal-body -->
	      		<div class="modal-footer">
	        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal" id="deletePageCancel">Cancelar</button>
	        		<button type="button" type="button" class="btn btn-primary btn-embossed" id="deletePageConfirm">Eliminar</button>
	      		</div>
	    	</div><!-- /.modal-content -->
	  	</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
	...
	<div class="modal fade small-modal" id="deleteElement" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			¿Seguro que desea eliminar este elemento? Una vez eliminado, no puede ser recuperado.
	      		</div><!-- /.modal-body -->
	      		<div class="modal-footer">
	        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal" id="deletePageCancel">Cancelar</button>
	        		<button type="button" type="button" class="btn btn-primary btn-embossed" id="deleteElementConfirm">Eliminar</button>
	      		</div>
	    	</div><!-- /.modal-content -->
	  	</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
	...
	<div class="modal fade" id="editContentModal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		...
	      		<div class="modal-footer">
	        		<button type="button" class="btn btn-default btn-embossed" data-dismiss="modal">Cancelar</button>
	        		<button type="button" type="button" class="btn btn-primary btn-embossed" id="updateContentInFrameSubmit">Guardar</button>
	      		</div>
	    	</div><!-- /.modal-content -->
	  	</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
  @yield('contenido')
	<div id="loader">
		{{HTML::image('backend/builder/images/loading.gif')}}
		Cargando elementos...
	</div>
	<div class="sandboxes" id="sandboxes" style="display: none"></div>
	<!-- Load JS here for greater good =============================-->
    {{HTML::script('backend/builder/js/jquery-1.8.3.min.js')}}
    {{HTML::script('backend/builder/js/jquery-ui.min.js')}}
    {{HTML::script('backend/builder/js/jquery.ui.touch-punch.min.js')}}
    {{HTML::script('backend/builder/js/bootstrap.min.js')}}
    {{HTML::script('backend/builder/js/bootstrap-select.js')}}
    {{HTML::script('backend/builder/js/bootstrap-switch.js')}}
    {{HTML::script('backend/builder/js/flatui-checkbox.js')}}
    {{HTML::script('backend/builder/js/flatui-radio.js')}}
    {{HTML::script('backend/builder/js/jquery.tagsinput.js')}}
    {{HTML::script('backend/builder/js/flatui-fileinput.js')}}
    {{HTML::script('backend/builder/js/jquery.placeholder.js')}}
    {{HTML::script('backend/builder/js/jquery.zoomer.js')}}
    {{HTML::script('backend/builder/js/application.js')}}
    {{HTML::script('backend/builder/js/spectrum.js')}}
    {{HTML::script('backend/builder/js/chosen.jquery.min.js')}}
    {{HTML::script('backend/builder/js/redactor/redactor.min.js')}}
    {{HTML::script('backend/builder/js/redactor/table.js')}}
    {{HTML::script('backend/builder/js/redactor/bufferButtons.js')}}
    {{HTML::script('backend/builder/js/src-min-noconflict/ace.js')}}
    
    {{HTML::script('backend/builder/js/accordion.js')}}
    
    {{HTML::script('backend/builder/js/select2/select2.min.js')}}
    {{HTML::style('backend/builder/js/select2/select2.css')}}
    {{HTML::script('backend/builder/js/select2/select2.min.js')}}
    
    {{HTML::script('backend/builder/elements.json')}}
    {{HTML::script('backend/builder/js/builder.js')}}
    <script>
	    $(function(){
