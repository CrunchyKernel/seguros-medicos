				...
				@foreach($contenidosArray AS $contenido)
					<div class="blog_post">	
						<div class="blog_postcontent">
							<div class="image_frame small"><a href="{{URL::to('/'.$contenido->categoria()->get()[0]->categoria_alias.'/'.$contenido->alias)}}"><img src="{{((strlen($contenido->imagen_medium) > 0) ? $contenido->imagen_medium : asset('assets/images/preview_medium.png'))}}" title="{{$contenido->titulo}}"></a></div>
							<div class="post_info_content_small">
								...
								<h4><a href="{{URL::to('/'.$contenido->categoria()->get()[0]->categoria_alias.'/'.$contenido->alias)}}">{{str_limit($contenido->titulo, 100)}}</a></h4>
