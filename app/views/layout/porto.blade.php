...
<html lang="es">
	...
	<body>
														...
														<ul class="dropdown-menu">
															@foreach($finalMenu as $menu)
																@if(count($menu->submenu)==0)
																	<li><a href="{{URL::to('/' . $menu->url_amigable)}}" class="dropdown-item">{{$menu->titulo}}</a></li>
																@else
																	<li class="dropdown-submenu">
																		<a href="{{URL::to('/' . $menu->url_amigable)}}" class="dropdown-item dropdown-toggle">{{$menu->titulo}}</a>
																		<ul class="dropdown-menu">
																			@foreach($menu->submenu as $submenu)
																				@if(count($submenu->submenu)==0)
																					<li><a href="{{URL::to('/' . $submenu->url_amigable)}}" class="dropdown-item">{{$submenu->titulo}}</a></li>
																				@else
																					<li class="dropdown-submenu">
																						<a href="{{URL::to('/' . $submenu->url_amigable)}}" class="dropdown-item dropdown-toggle" data-toggle="dropdown">{{$submenu->titulo}}</a>
																						<ul class="dropdown-menu">
																							@foreach($submenu->submenu as $submenu2)
																								<li><a href="{{URL::to('/' . $submenu2->url_amigable)}}" class="dropdown-item">{{$submenu2->titulo}}</a></li>
																							@endforeach
																						</ul>
																					</li>
																				@endif
		...
		<script type="text/javascript" async src="https://d335luupugsy2.cloudfront.net/js/loader-scripts/e39fd218-d19b-4913-882d-ea20b8e25c92-loader.js" ></script>
	</body>
</html>
