...
$( window ).load(function() {
	...
	$.ajax({
			url : '../getPaginaDatos',
			data : { idPagina : $("#idPagina").val() },
			cache : false,
			dataType: "json",
			type : 'POST',
		}).done(function(response){
			var blocksElement = JSON.parse(response.secciones);
			var blocksFrame = JSON.parse(response.html);
			var x = 1;
			if( $('ul#page'+x).size() == 0 ) {
...
function setPendingChanges(v) {
	if( v == true ) {
		$('#savePage .bLabel').text("Guardar cambios (!)");
		pendingChanges = true;
	} else {
		$('#savePage .bLabel').text("No se han realizado cambios");
...
function allEmpty() {
	...
	if( allEmpty ) {
		$('a.actionButtons:not(#savePage)').each(function(){
			...
			if( ui.item.find('.frameCover').size() == 0 ) {
				if( ui.item.find('iframe').size() > 0 ) {//iframe thumbnails
					...
					if (typeof attr !== typeof undefined && attr !== false) {
						//sandboxed
						theID = getRandomArbitrary(10000, 1000000000);
						sandboxedFrame = $('<iframe src="'+ui.item.find('img').attr('data-src')+'" id="'+theID+'" sandbox="allow-same-origin"></iframe>');
						$('#sandboxes').append( sandboxedFrame );
						...
						ui.item.html('<iframe src="'+ui.item.find('img').attr('data-src')+'" scrolling="no" frameborder="0" data-sandbox="'+theID+'" '+loaderFunction_+'><iframe>');
					} else {
						ui.item.html('<iframe src="'+ui.item.find('img').attr('data-src')+'" scrolling="no" frameborder="0"><iframe>');
					}
					ui.item.find('iframe').uniqueId();
					ui.item.find('iframe').height(theHeight+"px");
					ui.item.find('iframe').css('background', '#ffffff url(../../backend/builder/images/loading.gif) 50% 50% no-repeat');
...
function styleClick(el) {
	...
	if( $(el).hasClass('fa') ) {
		...
		get = get.replace('fa-4x,', '');
		get = get.replace('fa-3x,', '');
		get = get.replace('fa-2x,', '');
		get = get.replace('fa-lg,', '');
		...
		for( x=0; x<_Elements.elements[key].length; x++ ) {
			...
			if( _Elements.elements[key][x].thumbnail == null ) {//we'll need an iframe
				...
				if( _Elements.elements[key][x].sandbox != null ) {
					...
					newItem = $('<li class="element '+niceKey+'"><img src="'+_Elements.elements[key][x].thumbnail+'" data-src="'+_Elements.elements[key][x].url+'" data-height="'+_Elements.elements[key][x].height+'" data-sandbox="" '+loaderFunction+'></li>')
				} else {
...
function savePage(e) {
	...
	$('#pageList > ul').each(function(){
		...
		$.ajax({
				url : '../guardarPagina',
				data : { idPagina : $("#idPagina").val(), builder : JSON.stringify(blocksElement), html : JSON.stringify(blocksFrame) },
				cache : false,
				//contentType : false,
				//processData : true,
				dataType: "json",
				...
				setPendingChanges(response.realizado)
			});
	});
	//page names
	var pageNames = [];
