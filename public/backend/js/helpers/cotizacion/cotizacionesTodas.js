...
var cotizacionesTodas = cotizacionesTodas || {
...
/*;(function($, window, undefined){
    ...
    $.extend(cotizacionesTodas, {   
        ...
        crearTabla: function(estatus){
            oTable = $('#listadoCotizaciones').DataTable({
                    ...
                    $(".campo").editable({
                            ...
                            success: function(response, newValue) {
                                if(response == true){
                                    ...
                                    cotizacionesTodas.actualizarTabla();
                ...
                "sAjaxSource": _root_+"cotizacion/getConsultaCotizacionesTodas",
                "bServerSide": true,
                    ...
                    $.ajax({
                        ...
                        success: function(response)
                        {
                            if(response.status == 'success'){
                                cotizacionesTodas.actualizarTabla(estatus);
                            }
                            swal({
    ...
    $('body').delegate('.eliminarCotizacion', 'click', function(event){
        ...
        cotizacionesTodas.eliminarCotizacion(idCotizacion, estatus);
    });
...
jQuery(document).ready(function() {
    //cotizacionesTodas.init();
    
    var tbl = $("#listadoCotizaciones2").DataTable({
		language:{url:'https://cdn.datatables.net/plug-ins/9dcbecd42ad/i18n/Spanish.json'},
		pageLength:100,
		order:[[0, 'desc']],
		ajax:_root_ + 'cotizacion/getConsultaCotizacionesTodas2',
		columnDefs:[
			{targets: [2, 6, 7], searchable: false}
		]
	});
	...
	$('body').on('click', '.eliminarCotizacion', function(e){
		...
		var data = 'pk=' + $(this).data('idcotizacion') + '&campo=estatus&value=7';
		$.ajax({
			url: _root_ + 'cotizacion/actualizarCotizacionCampos',
			method: 'POST',
			dataType: 'json',
			...
			success: function(data, status, jqXHR){
				Adminsis.notificacion("Actualizacion", "Campo actualizado", "stack_bottom_left", "success");
				$(td).find('a').data('value', 'No').html('No');
			},
	...
	$(".actualizarTabla").on("click", function(e){
		tbl.ajax.url(_root_ + 'cotizacion/getConsultaCotizacionesTodas2/' + $("#id_agente").val() + '/' + $("#estatus").val() + '/' + $("#valor").val()).load();
	});
});