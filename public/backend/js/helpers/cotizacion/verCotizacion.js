...
;(function($, window, undefined){
    ...
    $.extend(verCotizacion, {   
        ...
        enviarCotizacionEmail: function(dialog){
            if($('#idCotizacionEmail').val() > 0){
                $.ajax({
                        ...
                        data : { idCotizacionEmail : $('#idCotizacionEmail').val(), para : $(".tm-input").tagsManager('tags'), sa : $('#sa').val(), ded : $('#ded').val(), mensaje : $(".summernote-quick").summernote('code') },
                        cache : false,
                        beforeSend: function(){
        ...
        recotizar: function(button, primera){
	        ...
	        var url = _root_+'cotizacion/recotizar/' + idCotizacion + '/' + sa + '/' + ded;
	        if(primera==1)
	        	url += '/1';
	        $.ajax({
	            url: url,
	            ...
	            success: function(response)
	            {
	                if(response["esencial"]){
						$("#contado-" + sa + ded + "-esencial").html("<strong>" + response["esencial"]["contado"] + "</strong>");
						$("#semestral-1-" + sa + ded + "-esencial").html(response["esencial"]["semestral-1"]);
						$("#semestral-2-" + sa + ded + "-esencial").html(response["esencial"]["semestral-2"]);
						$("#trimestral-1-" + sa + ded + "-esencial").html(response["esencial"]["trimestral-1"]);
						$("#trimestral-2-" + sa + ded + "-esencial").html(response["esencial"]["trimestral-2"]);
						$("#mensual-1-" + sa + ded + "-esencial").html(response["esencial"]["mensual-1"]);
						$("#mensual-2-" + sa + ded + "-esencial").html(response["esencial"]["mensual-2"]);
					}
					if(response["optima"]){
						$("#contado-" + sa + ded + "-optima").html("<strong>" + response["optima"]["contado"] + "</strong>");
						$("#semestral-1-" + sa + ded + "-optima").html(response["optima"]["semestral-1"]);
						$("#semestral-2-" + sa + ded + "-optima").html(response["optima"]["semestral-2"]);
						$("#trimestral-1-" + sa + ded + "-optima").html(response["optima"]["trimestral-1"]);
						$("#trimestral-2-" + sa + ded + "-optima").html(response["optima"]["trimestral-2"]);
						$("#mensual-1-" + sa + ded + "-optima").html(response["optima"]["mensual-1"]);
						$("#mensual-2-" + sa + ded + "-optima").html(response["optima"]["mensual-2"]);
					}
					button.html(text);
	            }
	        });
    ...
    $('body').delegate('.agregarSeguimiento', 'click', function(event){
        ...
        swal({
            ...
            verCotizacion.agregarSeguimiento(idCotizacion, cotizacionEstatus, seguimiento, fechaProgramada+' '+horaProgramada);
        });
    });
    ...
    $('body').delegate('.actualizarIntegrantes', 'click', function(event){
        event.preventDefault();
        var integrantes = verCotizacion.validarIntegrantes();
        if(integrantes.length > 0){
            var form = $("#actualizarIntegrantesForm");
            if(form.valid() === true){
                $.ajax(_root_ + "cotizacion/actualizarIntegrantes",{
                                        data : $(form).serialize(),
                                        cache: false,
                                        timeout: 0,
                                        method: 'POST',
    ...
    $('body').delegate('.recotizarMapfre', 'click', function(event){
    	event.preventDefault();
        verCotizacion.recotizar($(this), 0);
    });
})(jQuery, window);
